        ...
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-sm p-5">
            <form method="POST" action="/platform/{{ $platform->id }}" enctype="multipart/form-data" class="admin-form">
                <div class="form-group">
                    ...
                    <textarea name="description" id="description" class="bg-gray-100 rounded border border-gray-400 leading-normal resize-none w-full h-20 py-2 px-3 font-medium placeholder-gray-700 focus:outline-none focus:bg-white"  placeholder='Description'>{{ $platform->description }}</textarea>  
                ...
                <div class="form-group">
                    <button type="submit" class="btn-default btn-large">Update platform</button>
                </div>
                {{ csrf_field() }}
            </form>
        </div>
